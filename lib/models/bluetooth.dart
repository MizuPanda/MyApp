...
// Start advertising the service
  ...
  static Future<void> startScanning(String typeConstant, List<BluetoothDevice> devices,Future<BluetoothDevice> Function(DocumentSnapshot, DiscoveredDevice) newDevice, Function notifyListeners, {bool? advertise}) async {
          ...
          if (uuid.contains(appUuid)) {
            ...
            element.device.serviceUuids.first.toString() != uuid)) {
              ...
              if(scannedUserData != null) {
                BluetoothDevice device = await newDevice(scannedUserData, scanResult);
                await device.awaitDevice();

                if (devices.every(
                        (element) => element.username != device.username)) {
                  devices.add(device);
                  notifyListeners();
                }
              }
    ...
    if (_scanSubscription != null) {
      await Future.delayed(const Duration(seconds: 7), () async {
        if (_scanSubscription != null) {
          ...
          await startScanning(typeConstant, devices, newDevice, notifyListeners, advertise: advertise);
        }
      });
    }
  }
...
class ConnectedUser {
  ...
  ConnectedUser({required this.data}) {
    username = data.data().toString().contains('username')? data.get('username'): '';
